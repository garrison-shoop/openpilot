...
# BluePilot: external storage panel
...
class DeveloperLayoutSP(DeveloperLayout):
  ...
  def _initialize_items(self):
    ...
    self.external_storage = ExternalStorageControl()  # BluePilot
    ...
    self.items: list = [self.show_advanced_controls, self.enable_github_runner_toggle, self.enable_copyparty_toggle, self.prebuilt_toggle, self.error_log_btn,
                        self.external_storage.item,]  # BluePilot: external_storage.item
  ...
  # BluePilot: probe the drive on open so the panel doesn't show a stale value
  def show_event(self):
    super().show_event()
  ...
  def _update_state(self):
    # BluePilot: cheap -- only compares a timestamp, real work happens on a worker thread.
