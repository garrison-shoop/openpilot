...
class ManagerProcess(ABC):
  ...
  name = ""
  shutting_down = False
  # BluePilot: revive crashed processes -- see ensure_running
  ...
  # BluePilot: restored alongside restart_if_crash
...
class PythonProcess(ManagerProcess):
  def __init__(self, name, module, should_run, enabled=True, sigkill=False, restart_if_crash=False):
    ...
    self.restart_if_crash = restart_if_crash  # BluePilot
...
def ensure_running(procs: ValuesView[ManagerProcess], started: bool, params: Params, CP: car.CarParams,
  ...
  for p in procs:
    if p.enabled and p.name not in not_run and p.should_run(started, params, CP):
      # BluePilot: revive a process that died on its own; start() below no-ops while
      # self.proc is set, even when that process is dead, so a crash would be permanent.
